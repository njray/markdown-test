...
[Azure Logic Apps][logic-apps] trigger the entire workflow by deploying the container instances on a schedule. Logic Apps provide an Azure Container Instances connector which allows an instance to be deployed upon a range of trigger events.
...
When batch scoring data with R models, consider how to parallelize the workload. The input data must be partitioned somehow such that the scoring operation can be distributed  across the cluster nodes. Ultimately, the choice must be made on a case-by-case basis, but consider the following:
